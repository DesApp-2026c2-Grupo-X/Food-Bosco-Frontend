...
const sizeGroup = (id: string, name: string, options: [string, number][]): ProductConfigGroup => ({
  id,
  name,
  type: 'single',
  required: true,
  min: 1,
  max: 1,
  options: options.map(([label, extraPrice], i) => ({
    id: `${id}-${i}`,
    name: label,
    extraPrice,
    available: true,
...
const extrasGroup = (id: string, options: [string, number][]): ProductConfigGroup => ({
  id,
  name: 'Extras',
  type: 'multiple',
  required: false,
...
export const MOCK_CATEGORIES: Category[] = [
  ...
  { id: '3', name: 'Acompañamientos', active: true },
  { id: '4', name: 'Bebidas', active: true },
  { id: '5', name: 'Postres', active: true },
]
