...
const productById = (id: string) => MOCK_PRODUCTS.find((product) => product.id === id)

const bestSeller = (id: string, position: number, quantity: number): ProductReportRow => ({
  position,
...
const highestRevenue = (id: string, position: number, revenue: number): ProductReportRow => ({
  position,
  product: productById(id)!,
  category: categoryFor(productById(id)!.categoryId),
  revenue,
...
const outOfStock = (id: string, quantity: number): OutOfStockRow => ({
  product: productById(id)!,
  category: categoryFor(productById(id)!.categoryId),
  quantity,
})
